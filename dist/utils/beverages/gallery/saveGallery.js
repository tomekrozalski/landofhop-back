...
function saveGallery(files, params, cb) {
  files.forEach((file, i) => {
    ...
    sharp(file.buffer).jpeg({}).resize(880).toBuffer((err, data) => {
      s3.upload({
    ...
    sharp(file.buffer).jpeg({}).resize(440).toBuffer(async (err, data) => {
      await s3.upload({
        Bucket: 'land-of-hop-images',
    ...
    sharp(file.buffer).jpeg({}).resize(220).toBuffer((err, data) => {
      s3.upload({
        Bucket: 'land-of-hop-images',
    ...
    sharp(file.buffer).webp({}).resize(880).toBuffer((err, data) => {
      s3.upload({
        Bucket: 'land-of-hop-images',
    ...
    sharp(file.buffer).webp({}).resize(440).toBuffer((err, data) => {
      s3.upload({
        Bucket: 'land-of-hop-images',
    ...
    sharp(file.buffer).webp({}).resize(220).toBuffer((err, data) => {
      s3.upload({
        Bucket: 'land-of-hop-images',
